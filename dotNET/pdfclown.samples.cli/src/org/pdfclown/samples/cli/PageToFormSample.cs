      ...
      : Sample
    {
        ...
        */
          ...
          )
        {
            ...
            _ = blockComposer.ShowText("Page-to-form sample");
            var breakSize = new SizeF(0, 8);
            _ = blockComposer.ShowBreak(breakSize);
            composer.SetFont(bodyFont, 8);
            _ = blockComposer.ShowText("This sample shows how to convert a page to a reusable form that can be placed multiple times on other pages scaling, rotating, anchoring and aligning it.");
            _ = blockComposer.ShowBreak(breakSize);
            _ = blockComposer.ShowText("On this page you can see some of the above-mentioned transformations:");
            breakSize.Width = 8;
            _ = blockComposer.ShowBreak(breakSize);
            _ = blockComposer.ShowText("1. anchored to the center of the page, rotated by 45 degrees counterclockwise, 300 point wide (preserved proportions);");
            _ = blockComposer.ShowBreak(breakSize);
            _ = blockComposer.ShowText("2. anchored to the bottom-left corner of the page, 300 point high (preserved proportions);");
            _ = blockComposer.ShowBreak(breakSize);
            _ = blockComposer.ShowText("3. anchored to the bottom-right of the page, 80 point wide and 200 point high (altered proportions).");
            blockComposer.End();
          ...
          )
        {
            ...
            var filePath = this.PromptFileChoice("Please select a PDF file to use as form");
            using (var formFile = new File(filePath))
            {
                ...
                this.Populate(document, form);

                // 5. Serialize the PDF file!
                _ = this.Serialize(file, "Page-to-form", "converting a page to a reusable form", "page to form");
            }
        }
    }
}