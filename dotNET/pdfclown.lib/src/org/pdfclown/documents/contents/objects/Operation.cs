...
*/
          ...
          )
        {
            ...
            if (this.operands != null)
            {
                _ = buffer.Append(" [");
                for (
                  int i = 0, count = this.operands.Count;
                  i < count;
                  i++
                  )
                {
                    ...
                    _ = buffer.Append(this.operands[i].ToString());
                }
                _ = buffer.Append("]");
            }
        ...
        public IList<PdfDirectObject> Operands => this.operands;

        public string Operator => this.@operator;
    }
}