//-----------------------------------------------------------------------
// <copyright file="FlateFilter.cs" company="">
...
namespace org.pdfclown.bytes.filters
{
    ...
    public sealed class FlateFilter : Filter
    {
        ...
        private byte[] DecodePredictor(byte[] data, PdfDictionary parameters)
        {
            ...
            var predictor = parameters.ContainsKey(PdfName.Predictor)
                ? ((PdfInteger)parameters[PdfName.Predictor]).RawValue
                : 1;
            ...
            var sampleComponentBitsCount = parameters.ContainsKey(PdfName.BitsPerComponent)
                ? ((PdfInteger)parameters[PdfName.BitsPerComponent]).RawValue
                : 8;
            var sampleComponentsCount = parameters.ContainsKey(PdfName.Colors)
                ? ((PdfInteger)parameters[PdfName.Colors]).RawValue
                : 1;
            var rowSamplesCount = parameters.ContainsKey(PdfName.Columns)
                ? ((PdfInteger)parameters[PdfName.Columns]).RawValue
                : 1;
            ...
            switch (predictor)
            {
                ...
                default: // PNG Predictors [RFC 2083] (byte-based).
                    ...
                    {
                        Array.Copy(
                            currentRowBytePredictions,
                            0,
                            previousRowBytePredictions,
                            0,
                            currentRowBytePredictions.Length);
                        Array.Clear(leftBytePredictions, 0, leftBytePredictions.Length);
                        for (var rowSampleByteIndex = sampleBytesCount; // Starts after the leading upper-left sample (see Paeth method).
                          rowSampleByteIndex < rowSampleBytesCount; rowSampleByteIndex++)
                        {
                            ...
                            switch (predictionMethod)
                            {
                                ...
                                case 3: // Average (predicts the average of the sample to the left and the sample above).
                                    sampleByte = byteDelta +
                                        ((int)Math.Floor(
                                            (leftBytePredictions[sampleByteIndex] +
                                                    previousRowBytePredictions[rowSampleByteIndex]) /
                                                2d));
                                    break;
                                case 4: // Paeth (a nonlinear function of the sample above, the sample to the left, and the sample to the upper left).
                                    ...
                                    if ((leftPrediction <= topPrediction) && (leftPrediction <= topLeftPrediction))
                                    {
                            ...
                            leftBytePredictions[sampleByteIndex] = currentRowBytePredictions[rowSampleByteIndex] =
                                (byte)sampleByte;
                        }
                    }
                    break;
